...
class InQuerySpec : FreeSpec({
  ...
  "Person IN (names) - simple" {
    val sql = Sql("SELECT id, firstName, lastName, age FROM Person WHERE firstName IN ${Params("Joe", "Jim")} ORDER BY id").queryOf<Person>()
    sql.sql shouldBe "SELECT id, firstName, lastName, age FROM Person WHERE firstName IN (?, ?) ORDER BY id"
  ...
  "Person IN (names) - single" {
    val sql = Sql("SELECT id, firstName, lastName, age FROM Person WHERE firstName IN ${Params("Joe")} ORDER BY id").queryOf<Person>()
    sql.sql shouldBe "SELECT id, firstName, lastName, age FROM Person WHERE firstName IN (?) ORDER BY id"
    sql.runOn(ctx) shouldBe listOf(
      Person(1, "Joe", "Bloggs", 111)
    )
  ...
  "Person IN (names) - empty" {
    val sql = Sql("SELECT id, firstName, lastName, age FROM Person WHERE firstName IN ${Params.empty()} ORDER BY id").queryOf<Person>()
    sql.sql shouldBe "SELECT id, firstName, lastName, age FROM Person WHERE firstName IN (null) ORDER BY id"
    sql.runOn(ctx) shouldBe listOf()
  }

  "Person IN (names) - empty list" {
    val names: List<String> = emptyList()
    Sql("SELECT id, firstName, lastName, age FROM Person WHERE firstName IN ${Params.list(names)} ORDER BY id").queryOf<Person>().runOn(ctx) shouldBe listOf()
  }
})
