...
class JdbcRowDecoder(
...
): RowDecoder<Connection, ResultSet>(ctx, module, initialRowIndex, api, decoders, columnInfos, type, endCallback) {
  ...
  companion object {
    operator fun invoke(
      ...
      return JdbcRowDecoder(ctx, module, 1, api, decoders, metaColumns, RowDecoderType.Regular, {})
    }
  }
  ...
  override fun cloneSelf(ctx: JdbcDecodingContext, initialRowIndex: Int, type: RowDecoderType, endCallback: (Int) -> Unit): RowDecoder<Connection, ResultSet> =
    JdbcRowDecoder(ctx, this.serializersModule, initialRowIndex, api, decoders, columnInfos, type, endCallback)
}
