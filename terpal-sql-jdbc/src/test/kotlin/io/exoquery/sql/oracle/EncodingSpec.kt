...
class EncodingSpec: FreeSpec({
  ...
  val ctx by lazy { DatabaseController.Oracle(ds, encodingConfig) }
  ...
  "encodes and decodes nullables - not nulls" {
    ...
    verifyOracle(res.first(), EncodingTestEntity.regular)
  }
  ...
  "encodes and decodes batch" {
    ...
    verifyOracle(res.get(0), EncodingTestEntity.regular)
    verifyOracle(res.get(1), EncodingTestEntity.regular)
  }

  "encodes and decodes nullables - nulls" {
    insert(EncodingTestEntity.empty).runOn(ctx)
    val res = Sql("SELECT * FROM EncodingTestEntity").queryOf<EncodingTestEntity>().runOn(ctx)
    verifyOracle(res.first(), EncodingTestEntity.empty)
  }
