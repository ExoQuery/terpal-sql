...
class EnumSerializationSpec : FreeSpec({
  ...
  "Enum serialization (INSERT/UPDATE)" - {
    "should insert enum values correctly" {
      ...
      // For this to work without the conversion to String we need: https://github.com/ExoQuery/Terpal/issues/17
      val newUserPrefString = newUserPref.name
      Sql("INSERT INTO user_notification_preferences (user_name, profile_type) VALUES ('Diana', $newUserPrefString::profile_type_enum)").action()
        .runOn(ctx)
    ...
    "should update enum values correctly" {
      ...
      val newStatusString = serializer<Status>().descriptor.getElementName(newStatus.ordinal)
      Sql("UPDATE order_status_test SET status = $newStatusString::status_enum WHERE id = 1").action().runOn(ctx)
    ...
    "should handle enum in WHERE clause" {
      ...
      val result =
        Sql("SELECT user_name FROM user_notification_preferences WHERE profile_type = $targetTypeString::profile_type_enum")
          .queryOf<UserName>()
          .runOn(ctx)
          .first()
  ...
  "Batch operations with enums" - {
    "should handle batch inserts with enums" {
      ...
      for (user in newUsers) {
        val usersName = user.userName
        // For this to work without the conversion to String we need: https://github.com/ExoQuery/Terpal/issues/17
        val usersProfileTypeName = user.profileType.name
        Sql("INSERT INTO user_notification_preferences (user_name, profile_type) VALUES ($usersName, $usersProfileTypeName::profile_type_enum)")
          .action()
          .runOn(ctx)
      }
