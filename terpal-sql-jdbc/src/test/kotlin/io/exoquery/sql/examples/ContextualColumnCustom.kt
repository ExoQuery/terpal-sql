...
object ContextualColumnCustom {
  ...
  suspend fun main() {
    ...
    val ctx = object: TerpalContext.Postgres(postgres.postgresDatabase) {
      override val additionalDecoders = super.additionalDecoders + JZonedDateTimeDecoder.map { zd -> MyDateTime(zd.year, zd.dayOfMonth, zd.monthValue, TimeZone.getTimeZone(zd.zone)) }
      override val additionalEncoders = super.additionalEncoders + JZonedDateTimeEncoder.contramap { md: MyDateTime -> ZonedDateTime.of(md.year, md.month, md.day, 0, 0, 0, 0, md.timeZone.toZoneId()) }
    }
